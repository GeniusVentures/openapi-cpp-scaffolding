/**
 * @file       catalog_plugin.hpp
 * @brief      Auto-generated OOP plugin wrapper for AI-Native Business OS API - Catalog
 * @date       auto-generated
 *
 * DO NOT EDIT — regenerated from catalog_openapi.json
 *
...
#ifndef CATALOG_PLUGIN_HPP
#define CATALOG_PLUGIN_HPP
...
///
/// Auto-generated plugin class for Catalog.
...
protected:
    ...
    REGISTER_PLUGIN(kDefaultPluginPriority, ({"/api/v1/products", "/api/v1/products/{productId}", "/api/v1/services", "/api/v1/services/{serviceId}"}))
...
///
/// CRUD handler for listProducts — list products entities
///
    virtual std::string listProducts(
        const RequestContext& /*ctx*/,
        const std::string& /*method*/,
        const std::string& urlPath,
        const std::string& body)
    {
        std::string prefix = KeyBuilder::MakePrefix("catalog", "products");
...
///
/// CRUD handler for createProduct — create a new products entity
///
    virtual std::string createProduct(
        const RequestContext& ctx,
        const std::string& /*method*/,
        const std::string& /*urlPath*/,
        const std::string& body)
    {
        try
        {
            ...
            auto keyResult = KeyBuilder::Build("catalog", "products", id);
...
///
/// CRUD handler for getProduct — get a single products entity
///
...
///
/// CRUD handler for updateProduct — update an existing products entity
///
    virtual std::string updateProduct(
        const RequestContext& /*ctx*/,
        const std::string& /*method*/,
        const std::string& urlPath,
        const std::string& body)
...
///
/// CRUD handler for deleteProduct — delete a products entity
///
    virtual std::string deleteProduct(
        const RequestContext& /*ctx*/,
        const std::string& /*method*/,
        const std::string& urlPath,
        const std::string& body)
...
///
/// CRUD handler for listServices — list services entities
///
    virtual std::string listServices(
        const RequestContext& /*ctx*/,
        const std::string& /*method*/,
        const std::string& urlPath,
        const std::string& body)
    {
        std::string prefix = KeyBuilder::MakePrefix("catalog", "services");
        auto scanResult = m_storage->Scan(prefix);
...
///
/// CRUD handler for createService — create a new services entity
///
    virtual std::string createService(
        const RequestContext& ctx,
        const std::string& /*method*/,
        const std::string& /*urlPath*/,
        const std::string& body)
    {
        try
        {
            ...
            auto keyResult = KeyBuilder::Build("catalog", "services", id);
            if (!keyResult.has_value())
            {
                return R"({"error":{"code":"INVALID_KEY","message":"Failed to build storage key"}})";
            }
...
///
/// CRUD handler for getService — get a single services entity
///
...
///
/// CRUD handler for updateService — update an existing services entity
///
    virtual std::string updateService(
        const RequestContext& /*ctx*/,
        const std::string& /*method*/,
        const std::string& urlPath,
        const std::string& body)
...
///
/// CRUD handler for deleteService — delete a services entity
///
    virtual std::string deleteService(
        const RequestContext& /*ctx*/,
        const std::string& /*method*/,
        const std::string& urlPath,
        const std::string& body)
    ...
    {
        pm->RegisterHandler("GET", "/api/v1/products", "listProducts",
            [this](const RequestContext& ctx, const std::string& m, const std::string& p, const std::string& b) {
                return listProducts(ctx, m, p, b);
            }, GetName(), kStubHandlerPriority);
        pm->RegisterHandler("POST", "/api/v1/products", "createProduct",
            [this](const RequestContext& ctx, const std::string& m, const std::string& p, const std::string& b) {
                return createProduct(ctx, m, p, b);
            }, GetName(), kStubHandlerPriority);
        pm->RegisterHandler("GET", "/api/v1/products/{productId}", "getProduct",
            [this](const RequestContext& ctx, const std::string& m, const std::string& p, const std::string& b) {
                return getProduct(ctx, m, p, b);
            }, GetName(), kStubHandlerPriority);
        pm->RegisterHandler("PATCH", "/api/v1/products/{productId}", "updateProduct",
            [this](const RequestContext& ctx, const std::string& m, const std::string& p, const std::string& b) {
                return updateProduct(ctx, m, p, b);
            }, GetName(), kStubHandlerPriority);
        pm->RegisterHandler("DELETE", "/api/v1/products/{productId}", "deleteProduct",
            [this](const RequestContext& ctx, const std::string& m, const std::string& p, const std::string& b) {
                return deleteProduct(ctx, m, p, b);
            }, GetName(), kStubHandlerPriority);
        pm->RegisterHandler("GET", "/api/v1/services", "listServices",
            [this](const RequestContext& ctx, const std::string& m, const std::string& p, const std::string& b) {
                return listServices(ctx, m, p, b);
            }, GetName(), kStubHandlerPriority);
        pm->RegisterHandler("POST", "/api/v1/services", "createService",
            [this](const RequestContext& ctx, const std::string& m, const std::string& p, const std::string& b) {
                return createService(ctx, m, p, b);
            }, GetName(), kStubHandlerPriority);
        pm->RegisterHandler("GET", "/api/v1/services/{serviceId}", "getService",
            [this](const RequestContext& ctx, const std::string& m, const std::string& p, const std::string& b) {
                return getService(ctx, m, p, b);
            }, GetName(), kStubHandlerPriority);
        pm->RegisterHandler("PATCH", "/api/v1/services/{serviceId}", "updateService",
            [this](const RequestContext& ctx, const std::string& m, const std::string& p, const std::string& b) {
                return updateService(ctx, m, p, b);
            }, GetName(), kStubHandlerPriority);
        pm->RegisterHandler("DELETE", "/api/v1/services/{serviceId}", "deleteService",
            [this](const RequestContext& ctx, const std::string& m, const std::string& p, const std::string& b) {
                return deleteService(ctx, m, p, b);
            }, GetName(), kStubHandlerPriority);
    }
...
public:
    ...
    std::string GetName() override { return "Catalog"; }
