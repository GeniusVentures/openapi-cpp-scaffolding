                                  ...
                                  const std::string& jsonBody)
{
    ...
    for (const auto& pe : m_patternHandlers)
    {
        ...
        // First matching pattern wins among equal priorities; any priority —
        // including stub priority 0 — must be selectable (regression: the
        // strict `>` against an initial bestPriority of 0 made every
        // generated by-id stub route undispatchable).
        if (bestMatch == nullptr || pe.priority > bestPriority)
        {
