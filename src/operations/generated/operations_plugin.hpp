/**
 * @file       operations_plugin.hpp
 * @brief      Auto-generated OOP plugin wrapper for AI-Native Business OS API - Operations
 * @date       auto-generated
 *
 * DO NOT EDIT — regenerated from operations_openapi.json
 *
...
#ifndef OPERATIONS_PLUGIN_HPP
#define OPERATIONS_PLUGIN_HPP
...
protected:
    ...
    REGISTER_PLUGIN(kDefaultPluginPriority, ({"/api/v1/tasks", "/api/v1/tasks/{taskId}"}))
...
///
/// CRUD handler for listTasks — list tasks entities
///
    virtual std::string listTasks(
        const RequestContext& /*ctx*/,
        const std::string& /*method*/,
        const std::string& urlPath,
        const std::string& body)
    {
        std::string prefix = KeyBuilder::MakePrefix("operations", "tasks");
...
///
/// CRUD handler for createTask — create a new tasks entity
///
    virtual std::string createTask(
        const RequestContext& ctx,
        const std::string& /*method*/,
        const std::string& /*urlPath*/,
        const std::string& body)
    {
        try
        {
            ...
            auto keyResult = KeyBuilder::Build("operations", "tasks", id);
...
///
/// CRUD handler for getTask — get a single tasks entity
///
...
///
/// CRUD handler for updateTask — update an existing tasks entity
///
    virtual std::string updateTask(
        const RequestContext& /*ctx*/,
        const std::string& /*method*/,
        const std::string& urlPath,
        const std::string& body)
...
///
/// CRUD handler for deleteTask — delete a tasks entity
///
    virtual std::string deleteTask(
        const RequestContext& /*ctx*/,
        const std::string& /*method*/,
        const std::string& urlPath,
        const std::string& body)
    ...
    {
        pm->RegisterHandler("GET", "/api/v1/tasks", "listTasks",
            [this](const RequestContext& ctx, const std::string& m, const std::string& p, const std::string& b) {
                return listTasks(ctx, m, p, b);
            }, GetName(), kStubHandlerPriority);
        pm->RegisterHandler("POST", "/api/v1/tasks", "createTask",
            [this](const RequestContext& ctx, const std::string& m, const std::string& p, const std::string& b) {
                return createTask(ctx, m, p, b);
            }, GetName(), kStubHandlerPriority);
        pm->RegisterHandler("GET", "/api/v1/tasks/{taskId}", "getTask",
            [this](const RequestContext& ctx, const std::string& m, const std::string& p, const std::string& b) {
                return getTask(ctx, m, p, b);
            }, GetName(), kStubHandlerPriority);
        pm->RegisterHandler("PATCH", "/api/v1/tasks/{taskId}", "updateTask",
            [this](const RequestContext& ctx, const std::string& m, const std::string& p, const std::string& b) {
                return updateTask(ctx, m, p, b);
            }, GetName(), kStubHandlerPriority);
        pm->RegisterHandler("DELETE", "/api/v1/tasks/{taskId}", "deleteTask",
            [this](const RequestContext& ctx, const std::string& m, const std::string& p, const std::string& b) {
                return deleteTask(ctx, m, p, b);
            }, GetName(), kStubHandlerPriority);
    }

public:
    ~OperationsPlugin() override = default;

    std::string GetName() override { return "Operations"; }
