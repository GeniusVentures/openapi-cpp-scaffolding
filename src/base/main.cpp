...
private:
    ...
    void HandleRequest()
    {
        ...
        // Retain the query string for handlers (paging limit/cursor) — never
        // appended to target; generated by-ID handlers extract IDs from the path
        const std::string queryString = (queryPos != std::string::npos)
            ? fullTarget.substr(queryPos + 1)
            : std::string();
