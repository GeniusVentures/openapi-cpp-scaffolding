...
TEST_F(PluginManagerTest, Route_PatternPathMatchesConcreteIdAtStubPriority)
{
    auto inventory = MakeMock("InventoryPlugin");
    pm.RegisterPlugin(inventory, 100, {"/api/inventory"});

    HandlerFn itf = [](const RequestContext& /*ctx*/, const std::string&, const std::string&, const std::string&) -> std::string { return "inventory-item"; };

    pm.RegisterHandler("GET", "/api/inventory/items/{id}", "get_item", itf, "InventoryPlugin", kStubHandlerPriority);

    // A concrete id path must dispatch through the {param} pattern even at
    // stub priority (0) — regression: pattern fallback required priority > 0,
    // so every generated by-id stub route 404'd against real requests.
    EXPECT_EQ(pm.Route(RequestContext{}, "GET", "/api/inventory/items/item-42", R"({"action":"get"})"), "inventory-item");
    // A path with an extra segment must still match nothing.
    EXPECT_TRUE(pm.Route(RequestContext{}, "GET", "/api/inventory/items/item-42/extra", R"({"action":"get"})").empty());
}
